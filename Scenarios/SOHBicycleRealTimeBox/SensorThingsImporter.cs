...
{
    private const string Host = "http://iot.hamburg.de/v1.1";
    private static SensorThingsClient _client;
    private static readonly GeoJsonReader Reader = new();

    public static FeatureCollection LoadData(DateTime? fromInput = null, DateTime? toInput = null,
        Envelope envelopeInput = null)
    {
        ...
        // File.WriteAllText("bicycle_rental_layer.geojson", geojson);
        ...
        string geometryWkt, params OdataQuery[] observationTemporalQuery)
    {
        ...
        var filterString = dataStreamTopic != null
            ? $"substringof(trim('{dataStreamTopic}'), name) and geo.intersects(location, geography'{geometryWkt}')"
            : null;
        var filterQuery = new OdataQuery
        {
            QueryFilter = filterString != null ? new QueryFilter(filterString) : null,
            QueryExpand = new QueryExpand(new[]
            {
